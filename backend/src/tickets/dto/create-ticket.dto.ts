...
export class CreateTicketDto {
    ...
    @IsOptional()
    @IsIn(['Bajo', 'Medio', 'Alto', 'Critico'], { message: 'La prioridad no es válida' })
    priority?: string;
    ...
    @ApiPropertyOptional({ description: 'ID del usuario afectado (opcional)', example: 3 })
    @IsOptional()
    @IsNumber({}, { message: 'El usuario afectado debe ser un número válido' })
